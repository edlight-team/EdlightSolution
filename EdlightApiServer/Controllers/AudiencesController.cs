...
namespace EdlightApiServer.Controllers
{
    ...
    {
        private readonly Audiences db;
        public AudiencesController() => db = new();
        ...
        public object Get([FromHeader] string id, [FromHeader] string condition)
        {
            try
            {
                ...
                {
                    AudiencesModel model = db.GetModels().FirstOrDefault(m => m.Id == new Guid(id));
                    if (model == null) return BadRequest("Аудитория не найдена в БД");
                    else return model;
                }
                else
                    return db.GetModels();
        ...
        [HttpPost]
        public object Post([FromBody] AudiencesModel model)
        {
        ...
        [HttpPut]
        public object Put([FromBody] AudiencesModel model)
        {
            try
            {
                if (!db.GetModels().Any(u => u.Id == model.Id)) return BadRequest($"Аудитория не найдена в БД");
