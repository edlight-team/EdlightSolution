...
namespace EdlightApiServer.Controllers
{
    ...
    {
        private readonly Groups db;
        public GroupsController() => db = new();
        ...
        public object Get([FromHeader] string id, [FromHeader] string condition)
        {
            try
            {
                ...
                {
                    GroupsModel model = db.GetModels().FirstOrDefault(m => m.Id == new Guid(id));
                    if (model == null) return NotFound("Группа не найдена в БД");
                    else return model;
                }
                else
                    return db.GetModels();
        ...
        [HttpPost]
        public object Post([FromBody] GroupsModel model)
        {
        ...
        [HttpPut]
        public object Put([FromBody] GroupsModel model)
        {
            try
            {
                if (!db.GetModels().Any(u => u.Id == model.Id)) return NotFound("Группа не найдена в БД");
