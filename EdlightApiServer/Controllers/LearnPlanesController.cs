...
namespace EdlightApiServer.Controllers
{
    ...
    [ApiController]
    public class LearnPlanesController : ControllerBase
    {
        private readonly LearnPlanes db;
        public LearnPlanesController() => db = new();
        ...
        public object Get([FromHeader] string id, [FromHeader] string condition)
        {
            try
            {
                ...
                {
                    LearnPlanesModel model = db.GetModels().FirstOrDefault(m => m.Id == new Guid(id));
                    if (model == null) return NotFound($"Учебный план не найден в БД");
                    else return model;
                }
                else
                    return db.GetModels();
        ...
        [HttpPost]
        public object Post([FromBody] LearnPlanesModel model)
        {
        ...
        [HttpPut]
        public object Put([FromBody] LearnPlanesModel model)
        {
            try
            {
                if (!db.GetModels().Any(u => u.Id == model.Id)) return NotFound($"Учебный план не найден в БД");
