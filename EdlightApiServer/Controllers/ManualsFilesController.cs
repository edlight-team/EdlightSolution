...
namespace EdlightApiServer.Controllers
{
    ...
    [ApiController]
    public class ManualsFilesController : ControllerBase
    {
        private readonly ManualsFiles db;
        public ManualsFilesController() => db = new();
        ...
        public object Get([FromHeader] string id, [FromHeader] string condition)
        {
            try
            {
                ...
                {
                    ManualsFilesModel model = db.GetModels().FirstOrDefault(m => m.ID == new Guid(id));
        ...
        [HttpPost]
        public object Post([FromBody] ManualsFilesModel model)
        {
            try
            {
                ManualsFilesModel posted = db.PostModel(model);
                return posted;
            }
            catch (Exception ex)
            {
        ...
        [HttpPut]
        public object Put([FromBody] ManualsFilesModel model)
        {
            try
            {
