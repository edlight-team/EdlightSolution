...
namespace EdlightApiServer.Controllers
{
    ...
    {
        private readonly Materials db;
        public MaterialsController() => db = new();
        ...
        public object Get([FromHeader] string id, [FromHeader] string condition)
        {
            try
            {
                ...
                {
                    MaterialsModel model = db.GetModels().FirstOrDefault(m => m.Id == new Guid(id));
                    if (model == null) return BadRequest("Материал не найден в БД");
                    else return model;
                }
                else
                    return db.GetModels();
        ...
        [HttpPost]
        public object Post([FromBody] MaterialsModel model)
        {
        ...
        [HttpPut]
        public object Put([FromBody] MaterialsModel model)
        {
            try
            {
                if (!db.GetModels().Any(u => u.Id == model.Id)) return BadRequest($"Материал не найден в БД");
