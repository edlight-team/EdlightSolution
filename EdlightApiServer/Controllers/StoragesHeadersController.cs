...
namespace EdlightApiServer.Controllers
{
    ...
    [ApiController]
    public class StoragesHeadersController : ControllerBase
    {
        private readonly StoragesHeaders db;
        public StoragesHeadersController() => db = new();
        ...
        public object Get([FromHeader] string id, [FromHeader] string condition)
        {
            try
            {
                ...
                {
                    StoragesHeadersModel model = db.GetModels().FirstOrDefault(m => m.ID == new Guid(id));
        ...
        [HttpPost]
        public object Post([FromBody] StoragesHeadersModel model)
        {
            try
            {
                StoragesHeadersModel posted = db.PostModel(model);
                return posted;
            }
            catch (Exception ex)
            {
        ...
        [HttpPut]
        public object Put([FromBody] StoragesHeadersModel model)
        {
            try
            {
